...
Le nouveau site ne récupère que les articles déjà dans Sanity. Tous les articles de l'ancien site doivent être recrées manuellement. À faire article par article :
...
- **Note technique :** après chaque ajout dans Sanity, un push GitHub est nécessaire pour déclencher le rebuild du site statique
...
- **ICAI** — mis à jour vers `https://icai-aviation.org` mais le site ne répondait pas. À revérifier

### Images de couverture des articles
- Vérifier que chaque article dans Sanity a une image de couverture
- En particulier les articles récemment créés/nettoyés

### SEO & métadonnées
- Vérifier les balises `<title>` et `<description>` de chaque page
- Vérifier que les slugs des articles correspondent aux URLs de l'ancien site (pour éviter les liens cassés)

### Liens internes
- Tester tous les liens de navigation (menu, footer, CTAs)
- Vérifier les liens vers la plateforme RCP (`rcp-landing-page.rcp.pub.airlab.thalesdigital.io`)
- Vérifier les liens vers Open-ATMS API

### Responsive / mobile
- Tester toutes les pages sur mobile et tablette
- Vérifier en particulier : ATM Ecosystem hub, page partners, page news (filtres)
...
### Page RCP in Action
- Ajouter une image de couverture pour chaque événement dans le carousel (si disponible)
...
### Déploiement & domaine
- Confirmer le nom de domaine final et la redirection depuis l'ancien site
- Tester le site sur le domaine final (HTTPS, redirections)

### Rebuild automatique depuis Sanity
- Mettre en place un webhook Sanity → GitHub Actions pour rebuilder automatiquement le site à chaque publication dans Sanity (actuellement, un push manuel est nécessaire)
...
- TDF rendu non-cliquable (lien supprimé)
- ICAI mis à jour vers `https://icai-aviation.org`
- Page news : tri chronologique des articles par catégorie corrigé
- ATM Ecosystem : ouverture automatique sur "Research Topics"
